...
<section class="bg-white py-16 border-t border-slate-100">
    <div class="max-w-7xl mx-auto px-6 text-center">
        <h2 class="text-3xl font-black text-slate-800 tracking-tight mb-2">Partner & Sponsor Resmi</h2>
        <p class="text-slate-500 text-sm mb-12">Dukungan ekosistem terbaik yang menguatkan aplikasi AmikomEventHub.</p>

        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 items-center justify-center">
            @forelse($partners as $partner)
                <div class="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col items-center justify-center transition hover:shadow-md">
                    <p class="font-bold text-lg text-indigo-950 mb-1">{{ $partner->name }}</p>
                    ...
                    @if($partner->link)
                        <a href="{{ $partner->link }}" target="_blank" class="text-xs text-blue-500 hover:underline mt-3">Kunjungi Website ↗</a>
                    @endif
                </div>
            @empty
                <div class="col-span-full text-center text-slate-400 text-sm">
