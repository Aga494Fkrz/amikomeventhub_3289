...
@section('title', 'Admin - Edit Data Partner')
...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-black text-slate-800 tracking-tight">Edit Data Partner</h1>
            <p class="text-slate-500 text-sm font-medium">Ubah informasi mitra kerja sama AmikomEventHub</p>
        </div>
    </div>
    ...
    <div class="bg-white p-8 rounded-2xl border border-slate-100 shadow-sm">
        <form action="{{ route('admin.partners.update', $partner->id) }}" method="POST" class="space-y-6">
            ...
            @method('PUT') <div>
                ...
                <input type="text" name="name" value="{{ old('name', $partner->name) }}" required placeholder="Masukkan nama resmi mitra" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-600 transition @error('name') border-red-500 @enderror">
            ...
            <div>
                ...
                <select name="category_id" required class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-600 bg-white transition @error('category_id') border-red-500 @enderror">
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $partner->category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="url" name="link" value="{{ old('link', $partner->link) }}" placeholder="https://example.com" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-indigo-600 transition @error('link') border-red-500 @enderror">
